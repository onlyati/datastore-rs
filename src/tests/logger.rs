...
mod test {
    ...
    fn test_log1() {
        ...
        let result = manager.write(LogItem::SetKey("/root/status/server1".to_string(), "alive".to_string()));
        assert_eq!(true, result.is_ok());

        let result = manager.stop();
        assert_eq!(true, result.is_ok());

        let result = manager.write(LogItem::SetKey("/root/status/server9".to_string(), "alive".to_string()));
